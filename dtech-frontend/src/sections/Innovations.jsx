...
const Innovations = () => {
      ...
      try {
        ...
        if (result.success && result.data.length > 0) {
          const formatted = result.data.slice(0, 3).map((item) => ({
            badge: item.is_featured ? 'FEATURED' : 'INNOVATION',
            title: item.title,
            context: item.subtitle || item.award_by || 'Innovation Project',
            desc: item.description || '',
            img: item.image_url || 'https://images.unsplash.com/photo-1565043589221-1a6fd9ae45c7?w=900&q=80',
            tags: [item.year ? `Since ${item.year}` : 'R&D', 'Innovation'],
          }))
  ...
  const innovation = displayInnovations[active] || defaultInnovations[0]

  return (
    <section style={{ background: '#000', paddingTop: '64px' }}>
      <PageHero
        image="https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1600&q=80"
        title="INNOVATIONS"
      />

      <section style={{ background: '#000', padding: '6rem 6vw' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <h2 style={{ fontFamily: 'Barlow Condensed', fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 800, color: '#fff', letterSpacing: '0.06em', marginBottom: '2rem', textTransform: 'uppercase' }}>
            g l o b a l   a c h i e v e m e n t
          </h2>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '3rem', flexWrap: 'wrap' }}>
                {displayInnovations.map((item, i) => (
                  <button
                    key={`${item.title}-${i}`}
                    onClick={() => setActive(i)}
                    style={{ padding: '0.75rem 1.5rem', fontFamily: 'Rajdhani', fontSize: '0.78rem', fontWeight: 700, letterSpacing: '0.08em', color: active === i ? '#000' : 'rgba(255,255,255,0.5)', background: active === i ? '#00b4d8' : 'transparent', border: active === i ? 'none' : '1px solid rgba(255,255,255,0.15)', cursor: 'pointer', textTransform: 'uppercase', borderRadius: '2px' }}
                  >
                    {item.badge}
                  </button>
                ))}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center' }} className="inn-grid">
                <div style={{ position: 'relative' }}>
                  <div style={{ paddingTop: '65%', position: 'relative', overflow: 'hidden', borderRadius: '4px' }}>
                    <img src={innovation.img} alt={innovation.title} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(20%)' }} />
                  </div>
                  <div style={{ position: 'absolute', top: '1rem', left: '1rem', padding: '0.4rem 0.9rem', background: '#00b4d8', color: '#000', fontFamily: 'Rajdhani', fontWeight: 800, fontSize: '0.75rem', letterSpacing: '0.1em', borderRadius: '2px' }}>
                    {innovation.badge}
                  </div>
                </div>

                <div>
                  <p style={{ fontFamily: 'Rajdhani', fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.2em', color: '#00b4d8', marginBottom: '0.75rem', textTransform: 'uppercase' }}>
                    {innovation.context}
                  </p>
                  <h3 style={{ fontFamily: 'Barlow Condensed', fontSize: 'clamp(1.3rem, 2.5vw, 2rem)', fontWeight: 800, letterSpacing: '0.03em', color: '#fff', lineHeight: 1.1, marginBottom: '1.25rem', textTransform: 'uppercase' }}>
                    {innovation.title}
                  </h3>
                  <p style={{ fontSize: '0.9rem', fontWeight: 300, lineHeight: 1.8, color: 'rgba(255,255,255,0.65)', marginBottom: '1.5rem' }}>
                    {innovation.desc}
                  </p>
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
                    {(innovation.tags || []).map((tag) => (
                      <span key={tag} style={{ padding: '0.3rem 0.75rem', background: 'rgba(0,180,216,0.1)', border: '1px solid rgba(0,180,216,0.3)', color: '#00b4d8', fontFamily: 'Rajdhani', fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.06em', borderRadius: '2px' }}>
                        {tag}
                      </span>
                    ))}
                  </div>
                  <button style={{ padding: '0.7rem 1.75rem', background: 'transparent', border: '1px solid #00b4d8', color: '#00b4d8', fontFamily: 'Rajdhani', fontWeight: 700, fontSize: '0.8rem', letterSpacing: '0.12em', borderRadius: '4px', cursor: 'pointer' }}>
                    LEARN MORE
                  </button>
                </div>
              </div>
