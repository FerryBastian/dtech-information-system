...
// Map porto1-6 ke allPortfolioItems by title
const homeToDetailMap = {
  'GT40 CHASSIS': 'gt40-chassis',
  'CARIO': 'cario',
  'AC COBRA CHASSIS': 'ac-cobra-chassis',
  'PARKING BOXX (SMART PARKING SYSTEM)': null,
  'AIREON': 'aireon',
  'HERO VTOL': 'hero-vtol',
}
...
const Portfolio = ({ variant = 'home' }) => {
  ...
  useEffect(() => {
    if (variant !== 'home') { setLoading(false); return }
    ...
    return (
      <section style={{ background: '#000', minHeight: '100vh', paddingTop: '64px' }}>
        <div style={{ textAlign: 'center', padding: '3rem 6vw 2rem' }}>
          <h1 style={{ fontFamily: 'Barlow Condensed', fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 900, color: '#fff', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{item.title}</h1>
        </div>
            ...
            {allPortfolioItems.map((item, i) => (
              <div key={item.id}
                ...
                <div style={{ paddingTop: '65%', position: 'relative', overflow: 'hidden' }}>
                  ...
                  {hovered === i && <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontFamily: 'Rajdhani', fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.15em', color: '#00b4d8' }}>VIEW PROJECT →</span></div>}
                </div>
          ...
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '4px', opacity: visible ? 1 : 0, transition: 'opacity 0.7s ease 0.2s' }} className="port-grid">
              {displayItems.slice(0, 6).map((item, i) => {
                const detailId = homeToDetailMap[item.title]
                return (
                  <div key={i}
                    onMouseEnter={() => setHovered(i)}
                    onMouseLeave={() => setHovered(null)}
                    onClick={() => detailId ? navigate(`/portfolio/${detailId}`) : navigate('/portfolio')}
                    style={{ cursor: 'pointer', background: '#111' }}
                  >
                    <div style={{ position: 'relative', paddingTop: '72%', overflow: 'hidden' }}>
                      <img src={item.image_url || item.img} alt={item.title}
                        style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', filter: hovered === i ? 'brightness(0.5)' : 'brightness(0.75) grayscale(15%)', transform: hovered === i ? 'scale(1.06)' : 'scale(1)', transition: 'all 0.5s ease' }} />
                      {hovered === i && (
