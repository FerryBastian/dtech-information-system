...
const getDefaultCategories = () => [
  ...
  },
  {
    ...
    gridImages: ['/Arumi1.jpg', '/Arumi2.jpg', '/Arumi3.jpg', '/Arumi4.jpg'],
  },
  {
    id: 'trainseat',
    label: 'TRAIN SEAT',
...
const Products = ({ withPageHero = false }) => {
  ...
  return (
    <section id="products" style={{ background: '#000' }}>
      {withPageHero ? (
        <PageHero image="https://images.unsplash.com/photo-1565043589221-1a6fd9ae45c7?w=1600&q=80" title="PRODUCTS" />
      ) : null}
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '0 6vw' }}>
        {displayCats.map((item, i) => (
          <button
            key={item.id}
            onClick={() => setActive(i)}
            style={{ fontFamily: 'Rajdhani', fontSize: '0.85rem', fontWeight: 700, letterSpacing: '0.12em', padding: '1.25rem 2rem', color: active === i ? '#00b4d8' : 'rgba(255,255,255,0.4)', borderBottom: active === i ? '2px solid #00b4d8' : '2px solid transparent', background: 'none', border: 'none', cursor: 'pointer', textTransform: 'uppercase', transition: 'color 0.2s', marginBottom: '-1px' }}
          >
            {item.label}
          </button>
        ))}
      ...
      {loading ? (
        ...
        <>
          {cat.id === 'machine' ? (
            <>
              <div style={{ position: 'relative', height: 'clamp(580px, 85vh, 860px)', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${cat.bg})`, backgroundSize: 'cover', backgroundPosition: '60% center', backgroundRepeat: 'no-repeat', filter: 'brightness(0.5) grayscale(10%)' }} />
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, rgba(0,0,0,0.72) 38%, rgba(0,0,0,0.1) 75%, transparent)' }} />
                ...
                <div style={{ position: 'absolute', bottom: '3rem', left: '6vw', maxWidth: '400px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-end', gap: '6px', marginBottom: '0.85rem' }}>
                    <span style={{ fontFamily: 'Barlow Condensed', fontWeight: 900, fontSize: 'clamp(1.6rem, 3.5vw, 2.8rem)', color: '#fff', letterSpacing: '0.04em', lineHeight: 1 }}>SUPER</span>
                    <span style={{ fontFamily: 'Barlow Condensed', fontWeight: 900, fontSize: 'clamp(1.6rem, 3.5vw, 2.8rem)', color: '#fff', letterSpacing: '0.04em', lineHeight: 1 }}>MILL</span>
                    <div style={{ display: 'flex', flexDirection: 'column', marginLeft: '3px', marginBottom: '3px', lineHeight: 1.1 }}>
                  ...
                  <p style={{ fontSize: 'clamp(0.78rem, 1.1vw, 0.9rem)', fontWeight: 300, color: 'rgba(255,255,255,0.78)', lineHeight: 1.75, marginBottom: '1.5rem' }}>
                    {cat.desc}
                  </p>

                  <button style={{ padding: '0.55rem 1.75rem', background: '#00b4d8', color: '#000', fontFamily: 'Rajdhani', fontWeight: 700, fontSize: '0.78rem', letterSpacing: '0.1em', border: 'none', borderRadius: '50px', cursor: 'pointer', transition: 'background 0.2s' }}>
                    Learn More
                  </button>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gridTemplateRows: '1fr 1fr', gap: '8px', padding: '0 4vw 2rem', height: '500px' }} className="arumi-grid">
                <div style={{ position: 'relative', gridRow: '1 / 3', overflow: 'hidden', borderRadius: '8px' }}>
                  ...
                  <div style={{ position: 'absolute', bottom: '1.5rem', left: '1.5rem' }}>
                    ...
                    <div style={{ fontFamily: 'Rajdhani', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.2em', color: 'rgba(255,255,255,0.65)', marginTop: '4px' }}>
                      A|UMI Moto Parts
                    </div>
                    ...
                    <button style={{ marginTop: '0.75rem', padding: '0.45rem 1.25rem', background: '#00b4d8', color: '#000', fontFamily: 'Rajdhani', fontWeight: 700, fontSize: '0.72rem', letterSpacing: '0.1em', border: 'none', borderRadius: '50px', cursor: 'pointer' }}>
                      Learn More
                    </button>
                  </div>
                </div>
          ...
          {cat.id === 'trainseat' ? (
            <>
              <div style={{ position: 'relative', height: 'clamp(580px, 85vh, 860px)', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${cat.bg})`, backgroundSize: 'cover', backgroundPosition: 'center', filter: 'brightness(0.45)' }} />
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, rgba(0,0,0,0.75) 38%, rgba(0,0,0,0.1) 75%, transparent)' }} />
                <div style={{ position: 'absolute', bottom: '3rem', left: '6vw', maxWidth: '400px' }}>
                  <h2 style={{ fontFamily: 'Barlow Condensed', fontSize: 'clamp(1.8rem, 3.5vw, 3rem)', fontWeight: 900, color: '#fff', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '0.85rem' }}>
                    TRAIN SEAT
                  </h2>
                  ...
                  <button style={{ padding: '0.55rem 1.75rem', background: '#00b4d8', color: '#000', fontFamily: 'Rajdhani', fontWeight: 700, fontSize: '0.78rem', letterSpacing: '0.1em', border: 'none', borderRadius: '50px', cursor: 'pointer' }}>
                    Learn More
                  </button>
                </div>
              </div>
      ...
      <style>{`
        @media(max-width:640px){.arumi-grid{grid-template-columns:1fr!important;grid-template-rows:auto!important;height:auto!important}}
